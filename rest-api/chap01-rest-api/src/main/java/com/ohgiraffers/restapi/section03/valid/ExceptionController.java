...
@ControllerAdvice  // 빈 등록도 되고, 하위의 익셉션 핸들러들이 전형적으로 기능되도록 함
public class ExceptionController {
    ...
    public ResponseEntity<ErrorResponse> handleUserException(UserNotFoundException e) {
        ...
        return new ResponseEntity<>(new ErrorResponse(code, description, detail), HttpStatus.NOT_FOUND);
                       // (new ErrorResponse(code, description, detail) : 바디에 넣어줌 / NOT_FOUND : 404에러
    }
    ...
    public ResponseEntity<ErrorResponse> methodValidException(MethodArgumentNotValidException e) {
        ...
        return new ResponseEntity<>(errorResponse, HttpStatus.BAD_REQUEST); // BAD_REQUEST : 파라미터로 넘어온 요청이 잘못됨
    }
