...
public class SimpleJPQLRepository {
    ...
    public String selectSingleMenuByTypedQuery() {
        ...
        TypedQuery<String> query = entityManager.createQuery(jpql, String.class);  // menuName 을 받을것이라서 String 타입 / 쿼리 객체를 생성
        String resultMenuName = query.getSingleResult();  // 쿼리를 실행해서 결과를 얻어옴
        return resultMenuName;
    }

    public Object selectSingleMenuByQuery() {
        String jpql = "SELECT m.menuName FROM Section01Menu as m WHERE m.menuCode = 8";
        Query query = entityManager.createQuery(jpql);   // 쿼리 객체를 생성
        Object resultMenuName = query.getSingleResult();  // 쿼리를 실행해서 결과를 얻어옴
        return resultMenuName;
    }
    ...
    public Menu selectSingleRowByTypedQuery() {
        ...
        TypedQuery<Menu> query = entityManager.createQuery(jpql, Menu.class);  // 쿼리 객체를 생성
        Menu resultMenu = query.getSingleResult();  // 쿼리를 실행해서 결과를 얻어옴
        return resultMenu;
    }
    ...
    public List<Integer> selectUsingDistinct() {
        String jpql = "SELECT DISTINCT m.categoryCode FROM Section01Menu as m";  // DISTINCT : 중복값 제거
