...
public class GroupFunctionRepository {
    ...
    public Long otherWithNoResult(int categoryCode) {  // NoResult : 결과 값이 없는
        String jpql = "SELECT SUM(m.menuPrice) FROM Section05Menu m WHERE m.categoryCode = :categoryCode";
        // COUNT 외의 다른 그룹함수는 결과 값이 없을 경우 Null 이 반환 되기 때문에 기본 자료형으로 다룰 경우 문제가 생긴다
        // Long 과 같이 Wrapper 클래스 자료형으로 다루어야 한다
