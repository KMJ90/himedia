...
public class EntityLifeCycleTests {
    ...
    void testTransient(int menuCode) {
        ...
        assertTrue(entityManager.contains(foundMenu)); // foundMenu 는 영속성 컨텍스트에서 관리되는 영속 상태의 객체
        assertFalse(entityManager.contains(newMenu));  // newMenu 는 영속성 컨텍스트에서 관리 되지 않는 비영속 상태의 객체
    }
    ...
    void testManagedSameEntityManager(int menuCode) {
        ...
        // when
        Menu menu1 = entityManager.find(Menu.class, menuCode);  // db 에서 정보를 찾음
        Menu menu2 = entityManager.find(Menu.class, menuCode);  // db 에서 찾아서 pc에 저장된 정보를 사용
    ...
    void testDetachAndMerge(int menuCode, int menuPrice) {
        ...
        entityManager.merge(foundMenu);  // 다시 관리 되는 객체로 merge 가 됨
        entityManager.flush();
    ...
    void testMergeUpdate(int menuCode, String menuName) {
        ...
        entityManager.merge(foundMenu);  // merge : 병합

        //then      ( 하양 민트초코죽 , DB의 원래 이름 )
    ...
    void testMergeSave() {
        ...
        EntityManager entityManager = EntityManagerGenerator.getInstance();
        EntityTransaction entityTransaction = entityManager.getTransaction();
        Menu foundMenu = entityManager.find(Menu.class, 20);  // 20 번에 해당하는 정보 찾아옴
        entityManager.detach(foundMenu);   // detach : 준영속 상태(관리하지 않는 상태)로 만듬

        // when
        entityTransaction.begin();
        foundMenu.setMenuName("치약맛 초코 아이스크림");  // 이름을 치약맛 초코 아이스크림 으로 변경
        foundMenu.setMenuCode(999);  // 존재하지 않는 코드값으로 변경
        entityManager.merge(foundMenu);  // 컨텍스트에도 없고, DB 에도 정보가 없으면, insert 해서 새로 생성한다
    ...
    void testClearPersistenceContext(int menuCode) {
        ...
        entityManager.clear();  // clear 로 영속화를 없앰(준영속화)
    ...
    void testRemoveEntity(int menuCode) {
        ...
        entityManager.flush();  // remove 후 flush 이후에는 null 값

        // then
        Menu refoundMenu = entityManager.find(Menu.class, menuCode);
        assertNull(refoundMenu);
        entityTransaction.rollback();  // rollback 으로 최종반영은 되지 않음
