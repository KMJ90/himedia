...
public class ResolverController {
    ...
    public String stringRedirectFlashAttribute(RedirectAttributes attr) {

        // 리다이렉트 처리 시 request scope 의 데이터는 공유 되지 않는다
        // session scope 에 너무 많은 데이터를 저장하는 것은 서버 성능에 영향을 준다
        // redirect 처리 시 전달하여 잠깐 사용하고 싶은 데이터가 있다면(ex. 저장 완료 alert 메세지 등)
        // RedirectAttribute 객체를 통해 잠시 세션에 저장 했다가 redirect 후 세션에서 제거되게 할 수 있다
        attr.addFlashAttribute("flashMessage", "리다이렉트 attr 사용하여 redirect");
