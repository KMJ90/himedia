...
// 1. 변수 중복 선언 금지
// 2. let, const 키워드로 선언된 변수는 같은 스코프 내에서 중복 선언을 허용하지 않음
let msg = "안녕하세요~";
// let msg = "안녕히 가세요~";

// 2. 블럭 레벨 스코프
// let 키워드로 선언한 변수는 모든 코드 블록(함수, if, for, while, try/catch 등)을 지역 스코프로 가진다
let i = 0;
for(let i = 0; i < 10; i++) {
...
if(true) {
    ...
    // 변수 호이스팅으로 아래 지역변수 y의 선언이 먼저 일어났기 때문에 
